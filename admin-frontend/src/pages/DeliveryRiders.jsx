...
import { Search, CheckCircle, Eye, Calendar } from "lucide-react";
import { getAllRiders, approveRider } from "../services/deliveryService";
...
// An inactive rider is one that was rejected (block/unblock no longer exists)
...
const DeliveryRiders = () => {
  ...
  const [modalRider, setModalRider] = useState(null); // { id, fullName }
  ...
  const confirmApprove = async () => {
    ...
    try {
      const data = await approveRider(modalRider.id);
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
              ...
              {riders.map((rider) => (
                <tr key={rider._id} className="border-t border-gray-100">
                  ...
                  <td className="px-4 py-3 text-gray-500">
                    {rider.createdAt ? new Date(rider.createdAt).toLocaleDateString("en-GB") : "—"}
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-1.5">
                      ...
                      {!rider.isApproved && rider.isActive && (
                        <button
                          onClick={() => setModalRider({ id: rider._id, fullName: rider.fullName })}
      ...
      {modalRider && (
        <ConfirmModal
          ...
          message={`${modalRider.fullName} will be approved and an email will be sent to the rider.`}
